...
from .connector_engine import ConnectorError, validate_public_url
...
def validate_connector_spec(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    return {**spec, "base_url": base_url, "operations": safe_operations,
            "execution_policy": {"allow_arbitrary_code": False, "require_https": True,
                                  "allow_redirects": False, "max_operations": 500}}
...
def execute_operation(spec: dict[str, Any], operation_name: str, *, path_params: dict[str, str] | None = None,
                      query: dict[str, str] | None = None, json_body: dict[str, Any] | None = None,
                      headers: dict[str, str] | None = None, auth_headers: dict[str, str] | None = None) -> dict[str, Any]:
    """Execute only a validated operation. Credentials are supplied separately and never stored in the spec."""
    ...
    response = requests.request(operation["method"], url, params=query or {}, json=json_body,
                                headers=merged_headers, timeout=(5, 30), allow_redirects=False, stream=True)
